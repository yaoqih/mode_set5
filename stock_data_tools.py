...
import warnings
import traceback
from config import other_file_path,basic_data_save_path
...
changeratelist=['turnover_rate','volume','amount','总市值','流通市值']
...
def last_train_data(initel=0):
    ...
    :initel:负为下一个，正为上一个
    '''
    trandcal=pd.read_csv(other_file_path+'trade_calendar.csv',encoding='utf-8')
    #提取所有开盘日
...
def 回测(df,dayafter,columns_list,rename=False):
    ...
    for i in range(len(df)):
        ...
        if code+'.csv' in os.listdir(data_path):
            df1=pd.read_csv(data_path+code+'.csv',encoding='utf-8')
            index=-1
            for j in range(len(df1)-1,-1,-1):
                if compare_date(df1['date'].iat[j],date):
                    index=j
                    break
            index+=dayafter
            if index+1>len(df1):
...
def download_basic_data(inputfile):
    try:
        ...
        if inputfile not in os.listdir(basic_data_save_path):
            ...
            df.to_csv(basic_data_save_path+inputfile,encoding='utf-8',index=False)
            if df.shape[0]>0 and len(df)>50 and compare_date(df['date'].iloc[-1],datetime)and compare_date(df['date'].iloc[-1],traintime):
                time.sleep(0.01)
            else:
...
def download_data(inputfile):
    try:
        ...
        if inputfile not in os.listdir(data_path):
            ...
            f.close()
            time.sleep(0.01)
            df=pd.read_csv(data_path+inputfile,encoding='utf-8')
            lock.release()
            df=df[~(df['pct_chg'].isin([None]))]
            df=df[~(df['open'].isin([0.0]))]
            df=df[~(df['open'].isin([0]))]
            ...
            if df.shape[0]>0 and len(df)>50 and  (df['日期'].iloc[0] ==trandtime or df['日期'].iloc[-1]==trandtime.replace('-','/')):
                ...
                df['复权pct_chg']=df['close']/df['前close']-1
                df['复权pct_chg'].iloc[0]=df['close'].iloc[0]/df['open'].iloc[0]-1
                df['复权因子'] = (df['复权pct_chg'] + 1).cumprod()
                initial_price = df.iloc[0]['close'] / (1 + df.iloc[0]['复权pct_chg'])  # 计算上市价格
                df['close_后复权'] = initial_price * df['复权因子']  # 相乘得到复权价
                df['open_后复权'] = df['open'] / df['close'] * df['close_后复权']
                df['high_后复权'] = df['high'] / df['close'] * df['close_后复权']
                df['low_后复权'] = df['low'] / df['close'] * df['close_后复权']
                rename_dict={'open_后复权':'open','close_后复权':'close','high_后复权':'high','low_后复权':'low','vol':'volume','成交额（千元）':'amount'}
                ...
                # rename_dict={'open':'open_后复权','clsoe':'close_后复权','high':'high_后复权','low':'low_后复权','volume':'vol（手）','amount':'成交额（千元）','pct_chg':'change','change':'pct_chg'}
                # df.rename(columns=rename_dict,inplace=True)
                stock=stock[50:]
                # lock.acquire()
                stock.to_csv(data_path+inputfile,encoding='utf-8',index=None,float_format='%.4f')
                # lock.release()
                return 0
            else:
...
def get_prise(code):
    ...
    try:
        ...
        nowprice=urlopen(req).read().decode('utf-8')
        if nowprice[-1]!=';':
            nowprice=urlopen(nowpriceurl).read().decode('utf-8')
...
def download_basic_data_all(data_path):
    ...
    :data_path:存放地址
    '''
    if 'SZ000001.csv' not in os.listdir(data_path)  or pd.read_csv(data_path+'SZ000001.csv',encoding='utf-8')['date'].iat[-1]!=last_train_data():
        delet(data_path)
    global basic_data_save_path
    basic_data_save_path = data_path
    df = pd.read_csv(other_file_path+'股票列表.csv',encoding='utf-8').code
    codelist = list(df)
    download_basic_data('1')
    download_basic_data('1')
    datetime_now=pd.read_csv(basic_data_save_path + 'SZ000001.csv', encoding='utf-8')['date'].iloc[-1]
    lock = Lock()
    ...
    for i in os.listdir(basic_data_save_path):
        if not compare_date(datetime_now,pd.read_csv(basic_data_save_path + i, encoding='utf-8')['date'].iloc[-1]):
            os.remove(basic_data_save_path+i)
    print('DownloadComplete')
def download_data_all():
    if 'SZ000001.csv' not in os.listdir(data_path) or pd.read_csv(data_path+'SZ000001.csv',encoding='utf-8').shape[1]<30 or pd.read_csv(data_path+'SZ000001.csv',encoding='utf-8')['date'].iat[-1]!=last_train_data():
        delet(data_path)
    df = pd.read_csv(other_file_path+'股票列表.csv',encoding='utf-8').code
    codelist = list(df)
    lock = Lock()
...
def get_allcode_pricenow(filename):
    ...
    for i in range(len(df)):
        ...
        df['现pct_chg'].iloc[i]=round((df['pricenow'].iloc[i]- price['before'])/price['before'],4)*100
        df['open'].iloc[i]=price['open']
        df['开盘pct_chg'].iloc[i]=round((price['open']- price['before'])/price['before'],4)*100
    for i in drop_list:
        df.drop(i,axis=0,inplace=True) 
    df=df.reindex(range(len(df)),method='bfill') 
    df['实际收益']=((df['pricenow']-df['open'])/df['open'])*100
    return df
def get_allcode_pricepast(df,file_path):
    ...
    for i in range(len(df)):
        ...
        if code+'.csv' in os.listdir(file_path):
            df1=pd.read_csv(file_path+code+'.csv',encoding='utf-8')
            df1['open'] = pd.to_numeric(df1['open'],errors='coerce')
            df1['close'] = pd.to_numeric(df1['close'],errors='coerce')
            for b in range(len(df1)-1,-1,-1):
                if compare_date(df1['date'].iloc[b],date):
                    break
            df['前close（复权）'].iloc[i]=df1['close'].iloc[b]
            if b+1<len(df1):
                df['open'].iloc[i]=df1['open'].iloc[b+1]
                df['close'].iloc[i]=df1['close'].iloc[b+1]
                df['high'].iloc[i]=df1['high'].iloc[b+1]
                df['low'].iloc[i]=df1['low'].iloc[b+1]
                df['开盘pct_chg'].iloc[i]=round((df1['open'].iloc[b+1]- df1['close'].iloc[b])/df1['close'].iloc[b],4)*100
            if b+2<len(df1):
                df['后open'].iloc[i]=df1['open'].iloc[b+2]
                df['pricenow'].iloc[i]=df1['close'].iloc[b+2]
                df['第三天收盘收益'].iloc[i]=((df1['close'].iloc[b+2]-df1['open'].iloc[b+1])/df1['open'].iloc[b+1])
            if b+3<len(df1):
                df['第四天开盘收益'].iloc[i]=((df1['open'].iloc[b+3]-df1['open'].iloc[b+1])/df1['open'].iloc[b+1])
                df['第四天收盘收益'].iloc[i]=((df1['close'].iloc[b+3]-df1['open'].iloc[b+1])/df1['open'].iloc[b+1])
            df.fillna(inplace=True,value=0.0)
    for i in drop_list:
        df.drop(i,axis=0,inplace=True) 
    df=df.reindex(range(len(df)),method='bfill') 
    df['第三天开盘收益']=((df['后open']-df['open'])/df['open'])
    return df
def delet(roottdir):
    '''
    删除路径中所有文件
...
def 数据周期转化_all(path,cycle,goal_path):
    ...
    :goal_path:目标储存位置
    '''
    if 'SZ000001.csv' not in os.listdir(goal_path)  or not compare_date(pd.read_csv(goal_path+'SZ000001.csv',encoding='utf-8')['date'].iat[-1],last_train_data()):
        delet(goal_path)
...
def 数据周期转化(filename):
    try:
        ...
        for i in range(len(df)-1,cycle,-1):
            df1['date'].iloc[a]=df['date'].iloc[i]
            df1['close'].iloc[a]=df['close'].iloc[i]
            df1['open'].iloc[a]=df['open'].iloc[i-cycle+1]
            df1['high'].iloc[a]=max(list(df['high'].iloc[i-cycle+1:i+1]))
            df1['low'].iloc[a]=min(list(df['low'].iloc[i-cycle+1:i+1]))
            df1['前close'].iloc[a]=df['前close'].iloc[i-cycle+1]
            df1['change'].iloc[a]=np.sum(list(df['change'].iloc[i-cycle+1:i+1]))
            df1['turnover_rate'].iloc[a]=np.average((list(df['turnover_rate'].iloc[i-cycle+1:i+1])))
            df1['vol'].iloc[a]=np.sum((list(df['vol'].iloc[i-cycle+1:i+1])))
            df1['amount'].iloc[a]=np.sum((list(df['amount'].iloc[i-cycle+1:i+1])))
            df1['总市值'].iloc[a]=np.average((list(df['总市值'].iloc[i-cycle+1:i+1])))
            df1['流通市值'].iloc[a]=np.average((list(df['流通市值'].iloc[i-cycle+1:i+1])))
            a=a+1
        df1.drop(df1[df1['date']==0].index,inplace=True)
        df1['股票代码']=df['股票代码'].iloc[-1]
        df1['名称']=df['名称'].iloc[-1]
        df1['pct_chg']=(df1['close']-df1['close'].shift(periods=-1))/df1['close'].shift(periods=-1)
        df1.sort_values(by=['date'], inplace=True)
        df1.to_csv(goal_path+filename.split('\\')[-1],encoding='utf-8',index=None)
    except:
        print(filename)
        traceback.print_exc()
def 参数生成_all(path,goal_path):
    ...
    :goal_path:目标储存位置
    '''
    if 'SZ000001.csv' not in os.listdir(goal_path) or pd.read_csv(goal_path+'SZ000001.csv',encoding='utf-8').shape[1]<30 or pd.read_csv(goal_path+'SZ000001.csv',encoding='utf-8')['date'].iat[-1]!=last_train_data():
        delet(goal_path)
    codelist = os.listdir(path)
    for i in range(len(codelist)):
...
def 参数生成(filename):
    ...
    stock=stock[50:]
    # lock.acquire()
    stock.to_csv(goal_path+filename.split('\\')[-1],encoding='utf-8',index=None,float_format='%.4f')
    # lock.release()
    return 0
def 采样_all(path,goal_path,name,date_remain,阈值,manage):
    ...
    if manage==2 or manage==0:
        ...
        pool.join()
def 采样方法(df,阈值):
    df['复权pct_chg下']=(df['close'].shift(-2)-df['open'].shift(-1))/df['open'].shift(-1)
    # df['target']=df['复权pct_chg下']
    df.loc[df['复权pct_chg下']>=阈值, 'target'] = 1
    df.loc[df['复权pct_chg下']<阈值, 'target'] = 0
    return df
def 采样(massage):
    '''
    没表头
    ...
    df=pd.read_csv(origin_data[:-12]+'SZ000001.csv',encoding='utf-8')
    date=df['date'].iloc[-date_remain]
    try:
        df=pd.read_csv(origin_data,encoding='utf-8')
        df=df[~(df['close'].isin([0.0]))]
        df=df[~(df['close'].isin([0]))]
        if len(df)>60 and len(df)>date_remain and compare_date(df['date'].iloc[-date_remain],date) :
            df=采样方法(df,阈值)
            df=df[~(df['close'].isin([0.0]))]
            df=df[~(df['close'].isin([0]))]
            df=df.iloc[3:-date_remain,:]
            lock.acquire()
            df.to_csv(goal_path+str(阈值)+','+str(date_remain)+'.csv',encoding='utf-8',index=False,mode='a',header=None)
            lock.release()
    except:
        print(massage)
def 采样2(massage):
    ...
    try:
        ...
        if len(df)>60 and len(df)>date_remain and compare_date(df['date'].iloc[-date_remain],date) :
            df=采样方法(df,阈值)
            df=df.iloc[3:-date_remain,:]
            df.to_csv(goal_path+str(阈值)+','+str(date_remain)+'.csv',encoding='utf-8',index=False,mode='a')
    except:
        print(massage)
def 测试采样(massage):
    '''
    ...
    try:
        df=pd.read_csv(origin_data,encoding='utf-8')
        if len(df)>60 and len(df)>date_remain and compare_date(df['date'].iloc[-1],date) :
            df=采样方法(df,阈值)
            df=df.iloc[-date_remain:-2,:]
            lock.acquire()
            df.to_csv(goal_path+str(阈值)+','+str(date_remain)+'测试'+'.csv',encoding='utf-8',index=False,mode='a',header=None)
            lock.release()
    except:
        print(massage)
def 测试采样2(massage):
    ...
    try:
        ...
        if len(df)>60 and len(df)>date_remain and compare_date(df['date'].iloc[-1],date) :
            df=采样方法(df,阈值)
            df=df.iloc[-date_remain:-2,:]
            df.to_csv(goal_path+str(阈值)+','+str(date_remain)+'测试'+'.csv',encoding='utf-8',index=False,mode='a')
    except:
        print(massage)